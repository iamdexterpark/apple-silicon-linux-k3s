...
| [`preflight.sh`](preflight.sh) | Verify the toolchain (`python3` + the deliverable gate's needs). | `validate.sh` |
...
| [`gates/provisioning.sh`](gates/provisioning.sh) | Deliverable gate for this repo: `bash -n` + `shellcheck` on the bring-up scripts, `ruby -c` + `cookstyle` on the Chef cookbook, JSON-validity on node attributes. | `validate.sh` / `preflight.sh` |

The deliverable here is **bare-metal provisioning** (`provisioning/` — shell + Chef), so `validate.sh`
selects [`gates/provisioning.sh`](gates/provisioning.sh). The `gates/manifests.sh` and
`gates/terraform.sh` gates ship with the template for other deliverable types and stay dormant unless
a `manifests/` or `terraform/` dir is present.
