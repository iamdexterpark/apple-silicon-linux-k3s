...
### 10a. Field-Tested Defects & Quirks (proven on real Asahi hardware)

The defects below were hit during a live multi-node bring-up on Apple Silicon and are fixed in this
repo's cookbook/scripts. They are called out because most will silently reproduce on identical
hardware and several **mask as success** (nodes show `Ready` while the cluster is broken).

| # | Reality | How it bites | Fix (in this repo) |
|---|---|---|---|
| D1 | **Fedora 40+/Asahi is dnf5 with no yum-compat libs.** | A bare Chef `package` resolves to `yum_package` → "cannot find yum libraries" → converge fails. | Use `dnf_package` explicitly (cookbook: firewalld, openssh-server, chrony). |
| D2 | **Fedora Server's default firewalld zone is `FedoraServer`, not `public`.** | The NIC lives in `FedoraServer`; ports opened in `public` do nothing → 6443 stays DROPped → agents never join → **cluster silently never forms** while the firewall *looks* configured. | Resolve `firewall-cmd --get-default-zone` at converge (`ruby_block` → `node.run_state`); apply target + ports to that zone. (`not_if`/`only_if` reject `lazy{string}` — use a plain block; `command` can take `lazy{}`.) |
| D3 | **Opening 8472/udp is necessary but NOT sufficient (post-decap).** | flannel decapsulates VXLAN and forwards inner pod traffic on `cni0`/`flannel.1`; in "no zone" they hit the default DROP → ALL cross-node pod-to-pod + ClusterIP traffic dropped. Nodes still `Ready` (kubelet→api on the underlay), so it masks (often surfaces as a MetalLB webhook timeout). | Put `cni0` + `flannel.1` and the pod (`10.42/16`) + service (`10.43/16`) CIDRs in firewalld's built-in `trusted` zone. |
| D4 | **K3s MERGES `config.yaml` AND systemd `ExecStart` flags** — neither overrides the other. | A `node-ip` set in both → `<ip>,<ip>` → kubelet rejects ("single IP or dual-stack pair") → kubelet dies → k3s crashloops; API flaps so `kubectl` intermittently works, masking it. | Pick ONE source. This repo's source of truth is `INSTALL_K3S_EXEC`; do not also write those flags into a config.yaml. Tell: `systemctl is-active k3s` == `activating` + journal "Shutdown request received: kubelet exited". |
| D5 | **Harden-before-key locks you out.** | Real first contact is password-only; disabling `PasswordAuthentication` before installing a key bricks remote access. | Install `authorized_keys` first; `disable_password` only takes effect once a key is proven present (cookbook lockout guard). |
| D6 | **Longhorn's backup engine SIGSEGVs on aarch64 16K-page kernels** (Asahi). | Configuring an S3 backup target crashes the engine (upstream longhorn#12929, OPEN, fix milestone v1.14.0 — no released fix as of v1.12.x). | The **core RWO data plane works** (PVCs bind, replicas heal). Set `backup-target: ""` explicitly; do NOT configure S3 backups until Longhorn ≥ v1.14.0. Use volumes + local snapshots. |
| D7 | **MetalLB behind a branch MX: L2 yes, BGP no.** | Meraki MX appliance BGP needs Auto-VPN + passthrough/concentrator mode; on a routed/NAT branch MX the API returns `400 ... only supported for VPN participants` / `... without passthrough mode enabled`. | Use MetalLB **L2 mode** behind a branch MX (the VIP pool must share the nodes' L2 segment — a node ARPs for the VIP). BGP/ECMP needs a dedicated concentrator or an L3 switch. |

**Environmental facts worth knowing (not bugs):** 16K-page kernel (`PAGE_SIZE 16384`, `...aarch64+16k`) is
the Asahi differentiator and the root of D6; `google-cloud-cli` is not in Fedora repos (add Google's
`cloud-sdk-el9-aarch64` repo in bootstrap if needed); `wheel` is not NOPASSWD by default, so scripts use
a `SUDO=""; [ "$(id -u)" -eq 0 ] || SUDO="sudo"` shim to run both as the root timer and interactively.
